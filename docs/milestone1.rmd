...
```{r}
# First we read in the dataset from the folder '\data' in our repository.
df <- read_csv("../data/fire_archive_M6_96619.csv")
```


### Research Question

In this project, we will seek to determine the relationship between a factor (day/night) and the intensity (brightness/bright_t31 and frp) of the wildfire, using the logistic regression.

### Plan of Action

With our research question, we are only interested in the fire intensity and the day/night. We will ignore the other variables for the purposes of this analysis. After data wrangling, we will perform a logistic regression analysis because of the nature of the factor (not numeric) and plot the relevant results. 
